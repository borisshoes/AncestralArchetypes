...
public class GlowBerryShieldLoginCallback extends LoginCallback {
   
   private float hearts;
   
   public GlowBerryShieldLoginCallback(){
      super(Identifier.fromNamespaceAndPath(AncestralArchetypes.MOD_ID, ArchetypeRegistry.BERRY_EATER.id()));
   }
   
   public GlowBerryShieldLoginCallback(ServerPlayer player, float hearts){
      this();
   ...
   public void onLogin(ServerGamePacketListenerImpl netHandler, MinecraftServer server){
      ...
      if(player.getStringUUID().equals(playerUUID)){
         ...
         if(player.getAbsorptionAmount() != 0){
            SoundUtils.playSongToPlayer(player, SoundEvents.SWEET_BERRY_BUSH_PICK_BERRIES, 1.0f, .3f);
         }
         MinecraftUtils.removeMaxAbsorption(player, Identifier.fromNamespaceAndPath(AncestralArchetypes.MOD_ID, ArchetypeRegistry.BERRY_EATER.id()),hearts);
   ...
   public boolean combineCallbacks(LoginCallback callback){
      if(callback instanceof GlowBerryShieldLoginCallback newCallback){
   ...
   public LoginCallback makeNew(){
      return new GlowBerryShieldLoginCallback();
   }
}
