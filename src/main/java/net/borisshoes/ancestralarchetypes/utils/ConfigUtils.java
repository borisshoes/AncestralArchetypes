...
public class ConfigUtils {
   ...
   public void read(){
      try(BufferedReader input = new BufferedReader(new InputStreamReader(new FileInputStream(file)))){
         logger.debug("Reading Ancestral Archetypes config...");
         ...
         logger.debug("Initialising Ancestral Archetypes config...");
         this.values.forEach(value -> value.value = value.defaultValue);
      }catch(IOException e){
         logger.fatal("Failed to load Ancestral Archetypes config file!");
         e.printStackTrace();
      }catch(Exception e){
         logger.fatal("Failed to parse Ancestral Archetypes config");
         e.printStackTrace();
      }
   }
   
   public void save(){
      logger.debug("Updating Ancestral Archetypes config...");
      try(BufferedWriter output = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file)))){
         output.write("# Ancestral Archetypes Configuration File" + " | " + new Date());
         output.newLine();
         output.newLine();
         ...
         logger.fatal("Failed to save Ancestral Archetypes config file!");
         e.printStackTrace();
      }
   }
   ...
   public LiteralArgumentBuilder<ServerCommandSource> generateCommand(){
      LiteralArgumentBuilder<ServerCommandSource> out =
            literal("archetypes").then(literal("config").requires(source -> source.hasPermissionLevel(4))
                  .executes(ctx -> {
