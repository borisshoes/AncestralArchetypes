...
public class WindChargeVolleyItem extends AbilityItem{
   ...
   public ActionResult use(World world, PlayerEntity user, Hand hand){
      ...
      SoundUtils.playSound(player.getEntityWorld(),player.getBlockPos(),SoundEvents.ENTITY_BREEZE_SHOOT, SoundCategory.PLAYERS,1f, 0.4f / (world.getRandom().nextFloat() * 0.4f + 0.8f));
   ...
   private void shootWindCharge(ServerPlayerEntity player, float firingErrorDeg, float power){
      ...
      WindChargeEntity windCharge = new WindChargeEntity(player, world, player.getEntityPos().getX(), player.getEyePos().getY(), player.getEntityPos().getZ());
