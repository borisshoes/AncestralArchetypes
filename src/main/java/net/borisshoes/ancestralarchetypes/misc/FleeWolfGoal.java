...
public class FleeWolfGoal<T extends LivingEntity> extends AvoidEntityGoal<T> {
   
   public FleeWolfGoal(AbstractSkeleton skeleton, Class<T> fleeFromType, float distance, double slowSpeed, double fastSpeed){
      super(skeleton, fleeFromType, distance, slowSpeed, fastSpeed, Objects.requireNonNull(EntitySelector.NO_CREATIVE_OR_SPECTATOR)::test);
   }
         ...
         if(vec3d == null){
            ...
            if(this.path != null){
               if(this.toAvoid instanceof ServerPlayer player){
                  ...
                  if(profile.hasAbility(ArchetypeRegistry.WOLF_SCARE)){
                     return true;
                  }
               }
            }
   ...
   public void start(){
      super.start();
      this.mob.playSound(SoundEvents.SKELETON_HURT, 0.75f, 1.5f);
      if(this.toAvoid instanceof ServerPlayer player){
         // TODO wtf sound registry?
         SoundUtils.playSongToPlayer(player, BuiltInRegistries.SOUND_EVENT.get(Identifier.withDefaultNamespace("entity.wolf_angry.growl")).get(), .25f, 1.5f);
      }
   }
}